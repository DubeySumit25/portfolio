...
function Field({ label, name, type = 'text', textarea, value, onChange, error }) {
  ...
  return (
    <div style={{ marginBottom: 'clamp(1rem, 2.5vw, 1.5rem)' }}>
      ...
      {error && (
        <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 'clamp(0.55rem, 1.5vw, 0.6rem)', color: 'var(--neon-pink)', marginTop: '0.3rem', letterSpacing: '0.1em' }}>
          ⚠ {error}
        </div>
      )}
    </div>
    ...
    <section className="section" style={{ position: 'relative' }}>
      ...
      <div className="section-inner" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 'clamp(1.5rem, 4vw, 3rem)' }}>
          ...
          {/* Left: info */}
          <div>
            <p style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 'clamp(0.9rem, 2.5vw, 1.05rem)', color: 'var(--text-dim)', lineHeight: 1.8, marginBottom: '2.5rem' }}>
            ...
            <div style={{ marginBottom: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                <div key={item.label} style={{ display: 'flex', gap: 'clamp(0.75rem, 2vw, 1rem)', flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 'clamp(0.55rem, 1.5vw, 0.6rem)', color: 'var(--text-ghost)', letterSpacing: '0.15em', minWidth: '70px', paddingTop: '2px', whiteSpace: 'nowrap' }}>
                    {item.label}
                  </span>
                  <span style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 'clamp(0.85rem, 2vw, 1rem)', color: item.highlight ? 'var(--neon-green)' : 'var(--text-primary)' }}>
                    {item.value}
                  </span>
                </div>
              ))}
              ...
              {SOCIALS.map(s => (
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer"
                  style={{
                    ...
                    color: 'var(--text-dim)',
                    textDecoration: 'none',
                    padding: 'clamp(0.35rem, 1vw, 0.5rem) clamp(0.65rem, 2vw, 0.9rem)',
                    border: '1px solid var(--border-dim)',
                    display: 'flex', alignItems: 'center', gap: 'clamp(0.3rem, 1vw, 0.4rem)',
                    transition: 'all 0.2s',
                    letterSpacing: '0.1em',
                    whiteSpace: 'nowrap',
                  }}
          ...
          <div className="cyber-card" style={{ padding: 'clamp(1.25rem, 3.5vw, 2rem)' }}>
            {status === 'success' ? (
              <div style={{ textAlign: 'center', padding: 'clamp(2rem, 4vw, 3rem) clamp(1rem, 3vw, 2rem)' }}>
                <div style={{ fontSize: 'clamp(1.8rem, 5vw, 2.5rem)', marginBottom: '1rem' }}>✓</div>
                <div style={{ fontFamily: 'Orbitron, monospace', fontSize: 'clamp(0.8rem, 2.5vw, 1rem)', color: 'var(--neon-green)', marginBottom: '0.75rem', letterSpacing: '0.15em' }}>
                  MESSAGE SENT
                </div>
                <p style={{ fontFamily: 'Rajdhani, sans-serif', color: 'var(--text-dim)', fontSize: 'clamp(0.85rem, 2vw, 1rem)' }}>
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 150px), 1fr))', gap: '0 clamp(1rem, 2vw, 1.5rem)' }}>
                ...
                {status === 'error' && (
                  <div style={{ fontFamily: 'Share Tech Mono, monospace', fontSize: 'clamp(0.6rem, 1.5vw, 0.65rem)', color: 'var(--neon-pink)', marginBottom: '1rem', padding: 'clamp(0.5rem, 2vw, 0.75rem)', border: '1px solid rgba(255,0,110,0.3)', background: 'rgba(255,0,110,0.05)' }}>
                ...
                <button
                  ...
                  onClick={onSubmit}
                  disabled={loading}
                  style={{ width: '100%', justifyContent: 'center', display: 'flex', alignItems: 'center', gap: '0.75rem', opacity: loading ? 0.7 : 1, fontSize: 'clamp(0.7rem, 2vw, 0.85rem)', padding: 'clamp(0.6rem, 2vw, 0.75rem) clamp(1rem, 3vw, 1.5rem)' }}
                >
                  {loading ? (
                    <>
